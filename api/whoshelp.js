// /api/whoshelp.js
//
// Slack slash-command handler: "/whoshelp who can help me build a chart"
//
// Flow:
//   1. Verify the request actually came from Slack (if SLACK_SIGNING_SECRET is set).
//   2. Fetch the same published roster CSV the whos-who site / api/directory use.
...
// 2. In Vercel project settings, confirm SHEET_CSV_URL is set (same one
//    api/directory.js already uses) — if not, add it.
// 3. Create a Slack app at https://api.slack.com/apps -> From scratch.
// 4. Under "Slash Commands", create /whoshelp with Request URL:
...
export default async function handler(req, res) {
  ...
  if (!query) {
    res.status(200).json({
      ...
      text: 'Ask me something like `/whoshelp who can help me build a chart`.',
    });
    return;
  }
  ...
  try {
    ...
    if (!sheetUrl) {
      res.status(200).json({
        response_type: 'ephemeral',
        text: 'Server is missing SHEET_CSV_URL — add it in Vercel project settings (same value api/directory.js uses), then redeploy.',
      });
      return;
    }

    const [csvResp, ] = await Promise.all([fetch(`${sheetUrl}${sheetUrl.includes('?') ? '&' : '?'}_ts=${Date.now()}`)]);
